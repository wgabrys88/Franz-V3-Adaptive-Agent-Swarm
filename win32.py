...
def _do_type_text(text: str) -> None:
    _vk_scan_fn = _user32.VkKeyScanW
    for char in text:
        vk_scan: int = _vk_scan_fn(ord(char))
        if vk_scan == -1:
            continue
...
def _do_press_key(key_name: str) -> None:
    vk_code: int | None = VK_MAP.get(key_name.strip().lower())
    if vk_code is None:
        return
    _key_event(vk_code)
...
def _do_hotkey(keys_str: str) -> None:
    ...
    for name in key_names:
        ...
        if vk_code is not None:
            ...
            vk_scan: int = _vk_scan_fn(ord(name))
            if vk_scan != -1:
...
def main() -> None:
    ...
    match command:
        case "capture":
            ...
            sys.stdout.buffer.write(_do_capture(region_arg, width_arg, height_arg))
            sys.stdout.buffer.flush()
        case "click":
            _do_click(get_arg("pos", "500,500"), get_arg("region", ""))
        case "double_click":
            _do_double_click(get_arg("pos", "500,500"), get_arg("region", ""))
        case "right_click":
            _do_right_click(get_arg("pos", "500,500"), get_arg("region", ""))
        case "type_text":
        ...
        case "scroll_up":
            _do_scroll(
                get_arg("pos", "500,500"), get_arg("region", ""),
                1, int(get_arg("clicks", str(CONFIG.scroll_default_clicks))),
            )
        case "scroll_down":
            _do_scroll(
                get_arg("pos", "500,500"), get_arg("region", ""),
        ...
        case "drag":
            _do_drag(get_arg("from_pos", "400,400"), get_arg("to_pos", "600,600"), get_arg("region", ""))
        case "cursor_pos":
            sys.stdout.write(_do_cursor_pos(get_arg("region", "")) + "\n")
            sys.stdout.flush()
        case "select_region":
